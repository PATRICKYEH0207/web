clc;clear;
fprintf('ResNet 10-fold')
path='.\imgae';
figuresdir_AUC='.\AUC\';
figuresdir_ConfusionMatrix='.\ConfusionMatrix\';
...
imds.ReadFcn = @(imgname) imresize(imread(imgname), [224,224]);
for time=1:times
    ...
    %erro image output
            for i=1:augimdsValidation.NumObservations
                ...
                if Compare==0
                    ...
                    if transfer==1
                        imwrite(image,['.\erro\ResNet101_',num2str(time),'_',char(Validation_Name),'_',name,ext]);
                    else
                        imwrite(image,['.\erro\nonResNet101_',num2str(time),'_',char(Validation_Name),'_',name,ext]);
                    end
                end
            end
            %-----confusion matrix-----%
            ...
            if transfer==1
                filename=['ConfusionMatrix_ResNet101_',num2str(time),'.png'];
                saveas(gcf,strcat(figuresdir_ConfusionMatrix,filename))
            else
                filename=['ConfusionMatrix_nonResNet101_',num2str(time),'.png'];
                saveas(gcf,strcat(figuresdir_ConfusionMatrix,filename))
            end
...
%-----roc cruve-----% 
            ...
            if transfer==1
                filename=['AUC_ResNet101_',num2str(time),'.png'];
...
%creat csv
if transfer==1
    fid = fopen('.\ACC\ResNet101net.csv','w');
    for i=1:4
...
%system('start chrome "localhost/transfer/cmd_matlab.php"');